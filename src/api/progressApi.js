  ...
  while (attempt < maxRetries) {
    try {
      ...
      const checkResponse = await fetch(
        `${apiBaseUrl}/api/Progress/user/${userId}/lesson/${lessonId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      ...
      if (checkResponse.status === 404) {
        ...
        if (!createResponse.ok) {
          ...
          throw new Error(
            `Không thể tạo tiến độ: ${createResponse.statusText} - ${JSON.stringify(errorData)}`
          );
        }
        const newProgressResponse = await fetch(
          `${apiBaseUrl}/api/Progress/user/${userId}/lesson/${lessonId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        ...
        if (!newProgressResponse.ok) {
          ...
          throw new Error(
            `Không thể lấy tiến độ sau khi tạo (lần ${attempt + 1}): ${newProgressResponse.statusText} - ${errorText}`
          );
        }
...
export const updateProgress = async (progressId, updateData) => {
  ...
  try {
    ...
    if (!updateResponse.ok) {
      ...
      throw new Error(
        `Không thể cập nhật tiến độ: ${updateResponse.statusText} - ${JSON.stringify(errorData)}`
      );
    }
